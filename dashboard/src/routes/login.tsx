...
export const Route = createFileRoute('/login')({
  beforeLoad: ({ search }) => {
    ...
    // A mobile login mints its own refresh-token session without disturbing
    // an existing browser or device session.
...
function LoginPage() {
  ...
  const handleSsoLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to SSO provider while keeping the validated native callback
      // in this browser tab for the eventual dashboard callback route.
      storeMobileAuthCallback(mobileAuthCallback)
