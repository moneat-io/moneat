...
function LinkedErrorsList({ eventIds }: { eventIds: string[] }) {
  const results = useQueries({
    queries: eventIds.map((eventId) => ({
      queryKey: ['event-issue', eventId],
      queryFn: () => api.getIssueIdForEvent(eventId),
      enabled: !!eventId,
    })),
  })
  ...
  return (
    <ul className="space-y-2">
      {eventIds.map((eventId, i) => {
        const issueId = results[i]?.data?.issueId
        return (
          <li key={eventId}>
            {issueId ? (
              <Link
          ...
          <div className="space-y-4">
            ...
            {replay.errorIds && replay.errorIds.length > 0 && (
              <Card>
                ...
                <CardContent>
                  <LinkedErrorsList eventIds={replay.errorIds} />
                </CardContent>
              </Card>
            )}
