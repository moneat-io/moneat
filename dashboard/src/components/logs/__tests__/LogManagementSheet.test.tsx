...
describe('LogManagementSheet', () => {
  ...
  it('runs pipeline, saved-view, metric, and monitor actions from the current explorer state', async () => {
    ...
    server.use(
      ...
      http.post(`${API_BASE}/v1/logs/monitors/from-query`, async ({request}) => {
        capturedBodies.push(await request.json() as Record<string, unknown>)
        return HttpResponse.json({
          name: 'High errors',
          query: 'service:api',
    ...
    fireEvent.click(screen.getByRole('button', {name: 'Create monitor draft'}))
    expect(await screen.findByText(/dashboard_alert_created/)).toBeInTheDocument()
