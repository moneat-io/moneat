...
Moneat is Sentry-protocol compatible, which means migrating from highlight.io is straightforward — you swap the SDK and point your DSN at Moneat. This guide provides a copy-paste AI prompt you can use to automate the migration across your codebase.
...
4. **Updating environment variables** — replace `HIGHLIGHT_PROJECT_ID` with a Sentry DSN

## Finding Your DSN

Before running the migration, grab your Moneat DSN from **Projects → [Your Project] → Settings**:
...
:::tip[For best results, run this prompt with access to your entire codebase (e.g. via GitHub Copilot Workspace, Cursor, or Claude Projects with your repo attached). Replace `YOUR_MONEAT_DSN` with your actual DSN before running.]
:::
...
| `@highlight-run/opentelemetry`  | `@sentry/opentelemetry`                    |
...
// TODO: No direct Sentry equivalent. Use a custom analytics tool or Sentry.captureMessage() as a workaround.
...
// TODO: Sentry does not have a built-in metrics API equivalent for these calls. Consider using a dedicated metrics service.
...
- **`H.track(event, metadata)`** — Sentry does not have a client-side event tracking API. Use `Sentry.captureMessage()` as a rough substitute, or integrate a dedicated analytics tool (e.g. PostHog, Amplitude).
- **`H.getSessionURL()`** — Sentry Replay does not expose a public URL for the current session. Remove or replace.
- **`H.recordMetric()` / `H.recordCount()` / `H.recordHistogram()`** — No direct SDK equivalent. Consider StatsD, Prometheus, or a dedicated metrics service.
...
| `H.track(event, metadata)` | ⚠️ No direct equivalent — see TODO note above |
...
| `H.getSessionURL()` | ⚠️ No equivalent |
| `H.recordMetric/Count/Histogram()` | ⚠️ No equivalent |
