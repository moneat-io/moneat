...
:::note Docker
If you're running the Agent in Docker, pass the profiling endpoint as an environment variable. The EPForwarder redirects require a `datadog.yaml` file (see [Agent Setup](/datadog-agent/agent-setup)):

```bash
docker run -e DD_APM_ENABLED=true \
           -e DD_DD_URL=https://api.moneat.io/dd \
           -e DD_APM_PROFILING_DD_URL=https://api.moneat.io/dd/profiling/v1/input \
...
services:
  datadog-agent:
    ...
    environment:
      ...
      - DD_DD_URL=https://api.moneat.io/dd
      - DD_APM_PROFILING_DD_URL=https://api.moneat.io/dd/profiling/v1/input
