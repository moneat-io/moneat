...
Database Monitoring requires the Datadog Agent integration, available on Enterprise plans.
:::

## Prerequisites

- A supported database (PostgreSQL, MySQL, SQL Server, or Oracle)
- The Datadog Agent deployed on a host with network access to the database (see [Agent Setup](/datadog-agent/agent-setup))
