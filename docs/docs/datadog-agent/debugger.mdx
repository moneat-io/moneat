...
Dynamic Instrumentation requires the Datadog Agent integration, available on Enterprise plans.
:::

## Prerequisites

- A supported application runtime (Java, Python, .NET, or Node.js)
- The Datadog Agent deployed alongside your application (see [Agent Setup](/datadog-agent/agent-setup))
...
java -javaagent:/path/to/dd-java-agent.jar \
  ...
  -Ddd.trace.agent.url=https://api.moneat.io/dd \
  -jar myapp.jar
```
...
DD_TRACE_AGENT_URL=https://api.moneat.io/dd \
...
:::tip
Start with log probes to capture context around a known issue. Once you identify the root cause, remove the probe to avoid unnecessary overhead.
:::
