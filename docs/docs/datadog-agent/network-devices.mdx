...
Network Device Monitoring requires the Datadog Agent integration, available on Enterprise plans.
:::

## Prerequisites

- SNMP-enabled network devices (routers, switches, firewalls, etc.)
- The Datadog Agent deployed on a host with network access to the devices (see [Agent Setup](/datadog-agent/agent-setup))
