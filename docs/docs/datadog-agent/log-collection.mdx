...
Logs ingested via the Datadog Agent are stored in ClickHouse alongside logs from other sources. Retention periods depend on your plan tier — check [Billing & Plans](/billing) for details.
