...
:::tip Debugging
Open your browser's Network tab and look for requests to `/api/{projectId}/analytics/event`. A `202 Accepted` response confirms events are being received.
:::
...
- The analytics ingestion endpoint (`/api/{projectId}/analytics/event`) is accessible from client browsers
