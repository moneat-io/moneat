...
|-------|--------|---------|
| Session ID (hash) | ✅ | `a1b2c3d4e5f6...` |
| Page path | ✅ | `/pricing` |
| Referrer source | ✅ | `Google` |
| UTM parameters | ✅ | `utm_source=newsletter` |
| Country code | ✅ | `US` |
| Region | ✅ | `California` |
| City | ✅ | `San Francisco` |
| Browser name | ✅ | `Chrome` |
...
| Device type | ✅ | `Desktop` |
| Screen width | ✅ | `1920` |
| Custom event name | ✅ | `Signup` |
| Custom properties | ✅ | `{ "plan": "pro" }` |

## What Is Never Stored

| Field | Stored | Notes |
|-------|--------|-------|
| IP address | ❌ | Used transiently for GeoIP + session hash, then discarded |
| User-Agent string | ❌ | Used transiently for UA parsing + session hash, then discarded |
| Cookies | ❌ | None set, none read |
| User IDs | ❌ | Not collected |
| Email addresses | ❌ | Not collected |
| Device fingerprints | ❌ | Not collected |
| localStorage/sessionStorage | ❌ | Not accessed |
...
This behavior is enabled by default and cannot be overridden.
...
Analytics events are stored in ClickHouse with a **730-day TTL** (2 years). After this period, events are automatically deleted by ClickHouse's TTL mechanism. Retention periods may vary by plan tier.
...
| Feature | Moneat | Google Analytics | Plausible | Matomo |
|---------|--------|------------------|-----------|--------|
| Cookies | None | Multiple | None | Optional |
| Personal data stored | None | Yes (IP, device ID) | None | Optional |
| Cookie banner needed | No | Yes | No | Depends |
| Hosted on your infra | Yes | No (Google servers) | Optional | Optional |
| Cross-site tracking | Impossible | Yes | No | No |
| Script size | ~670 B | ~45 KB | ~1 KB | ~22 KB |
