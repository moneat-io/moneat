...
| TASK-1.49 / 1.50 | Controlled rollout plus enterprise entitlements, quotas, and usage accounting | Planned |
...
| Slack identity mapping | **Self-asserted, not workspace-verified, not org-scoped** (`SlackUserMappings`, `/slack/link-user` — `IntegrationRoutes.kt:1017-1084`) | Verified against the bound workspace; org-scoped; guest/Connect/stale handling | TASK-1.9 |
...
- **INV-2 Org isolation:** every incident read/write is scoped by `organization_id` AND the object's id in the same predicate (`requireIncident(org,id)` — `IncidentCommandService.kt:859-865`; `versionPredicate` — `:892-895`). No cross-org object may be named, mutated, or referenced.
...
- **INV-6 Outbox at-least-once delivery with one effect per consumer:** domain events are written in the aggregate transaction; delivery is per-consumer, idempotent on `delivery_key`, leased, retried with backoff, and dead-lettered after `MAX_ATTEMPTS` (`IncidentOutboxService`).
...
| Org member (not involved) | ✅ | 🔶 (join first) | ⛔ | ⛔ | ✅ | ⛔ | ⛔ | ⛔ |
...
Resolution: Slack `(user_id, team_id)` → verify `team_id` is the workspace **bound** to the org → look up a **verified** mapping to a Moneat member → apply §4.1 as that member. Bot identity never confers user authority.
...
| Guest / single-channel guest | ⛔ by default | View only in invited channel | Guests are not members; never infer authority |
...
| **SLK-03** | Outbound Slack message duplication or loss (worker retries, Slack `rate_limited`, partial failure). | Channel comms | Duplicate/hidden updates, responder confusion. **High** | Outbox `delivery_key` is unique per consumer (`native_incident_outbox_deliveries` unique index — `IncidentModels.kt:156-157`); on retry, dedupe by delivery key; honor Slack `retry_after`; reconcile posted message ts. | TASK-1.10 | Sim: inject 429 + duplicate lease ⇒ exactly one channel post. DLQ depth metric. |
| **IDN-01** | Self-asserted `/slack/link-user` lets a user claim an arbitrary `slackUserId/teamId`, hijacking another Slack identity's inbound authority. | Identity mapping | Identity spoofing ⇒ unauthorized acks. **High** | Prove ownership (Slack OIDC / challenge) before binding; scope mapping to the bound workspace + org; unique per (org, slack_user). | TASK-1.9 | Test: link without proof ⇒ rejected. Unique-constraint test on (org, slack_user_id). |
...
| **PASS-03** | A rollout flag, UI check, API entitlement, or worker entitlement disagrees, exposing a partial native-incident/AI feature or processing queued work after access is removed. | Entitlements, incident data, usage | Entitlement bypass or orphaned side effects. **High** | Server-side entitlement is authoritative on every request and worker execution; rollout flags may restrict but never grant access; revoke/disable stops new work and safely terminalizes or quarantines queued deliveries. | TASK-1.49 / 1.50 / 1.48 | Combination test across flag on/off × entitlement on/off for API, Slack, AI tools, and workers; entitlement removal during a leased job produces no unauthorized side effect. |
...
|---|---|---|---|
| **Automated unit** | Visibility matrix (§4.1) per principal/row; org+id scoping; idempotency/fingerprint; version conflict; URL scheme allow-list; redaction golden tests; approval arg-hash/single-use/expiry | INC-01..06, TOK-01, AI-02/04/05 | TASK-1.2 / TASK-1.7 / TASK-1.9 / TASK-1.34 / TASK-1.35 + TASK-1.48 |
| **Integration sim** | Signed Slack interaction across workspace bindings (good/foreign-org/guest/Connect/unmapped/stale); replay same signed body; Twilio signature good/bad and callback replay; outbox duplicate-lease + 429; DLQ + replay; passthrough/native isolation; rollout × entitlement combinations | IDN-01..05, SLK-01..04, OUT-01/02, PASS-01..03, WFL-02, FIL-02 | Feature owners TASK-1.7 through TASK-1.10 / TASK-1.23 / TASK-1.24 / TASK-1.40 / TASK-1.46 / TASK-1.47 / TASK-1.49 / TASK-1.50; harness TASK-1.44; security sign-off TASK-1.48 |
...
**Always retain (structured):** actor principal (user resource UUID or bot/service identity), organization id, target object resource UUID + type, source/provenance (`REST`/`SLACK`/`INTEGRATION`/`WORKFLOW`/`IMPORT` — `IncidentTimelineProvenance`), decision (allow/deny + reason code), `command_key`/`delivery_key`, approval id (for AI mutations), and a correlation id spanning API → command → outbox → consumer.
...
- The incident timeline is itself an audit stream: every command writes a timeline event with provenance (`insertTimelineEvent` — `IncidentCommandService.kt:1041-1057`); AI mutations and approvals must append equivalently attributable audit records.
...
3. **Principal separation.** Slack **bot** and **user** are distinct principals with distinct grants; a workspace is an explicit binding principal, not an attribute; privileged workspace admins get install/scope authority only and never implicit Moneat authority. Slack-derived authority always resolves through a *verified* mapping to a Moneat member in the *bound* workspace.
4. **Scoping rule.** Every read and write is scoped by `organization_id` **and** the object id in one predicate; objects are addressed by `resource_id` UUID; the org is taken from the authenticated/bound context, never from client input. This includes AI snapshots and persisted approvals, even when they are also user-scoped. `PRIVATE` incidents are membership-gated across read, list, search, timeline, roster, notifications, status, and export.
...
7. **Delivery + reconciliation.** Domain events are written transactionally with the aggregate (outbox), delivered at-least-once per consumer, idempotent on `delivery_key`, retried with backoff, and dead-lettered after `MAX_ATTEMPTS`. The Postgres timeline is the source of truth; Slack/status channels are projections and are reconciled, never trusted back into state without provenance.
