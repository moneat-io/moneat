 ...
 */
class OnCallModule : EnterpriseModule, OnCallBridge {
    ...
    override fun startBackgroundJobs(application: Application) {
        ...
        slackUserGroupSyncService = SlackUserGroupSyncService(
            onCallScheduleService = onCallScheduleService,
            slackService = slackService,
            redisClient = redisClient
        )

        onCallHandoffService = OnCallHandoffService(
            onCallScheduleService = onCallScheduleService,
            pushNotificationService = pushNotificationService,
            redisClient = redisClient
        )
    ...
    override fun resolvePriority(organizationId: Int, severity: String): PriorityInfo? {
    ...
    override fun shouldEscalate(organizationId: Int, priorityLevel: String): Boolean {
        return businessHoursService.shouldEscalate(organizationId, priorityLevel)
    }
    ...
    override suspend fun triggerEscalation(
        ...
        val metadataMap: Map<String, JsonElement>? = metadata?.let {
            try { Json.decodeFromString<Map<String, JsonElement>>(it) } catch (_: Exception) { null }
        }
    ...
    override fun getIncident(incidentId: Int, userId: Int): IncidentInfo? {
    ...
    override fun acknowledgeIncident(incidentId: Int, userId: Int): Boolean {
        return incidentManagementService.acknowledge(incidentId, userId)
    }
}
