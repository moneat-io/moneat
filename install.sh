...
print_summary() {
  ...
  printf "    %bdocker compose logs -f%b                            # follow logs\n" "${DIM}" "${RESET}"
  printf "    %bdocker compose ps%b                                 # service status\n" "${DIM}" "${RESET}"
  printf "    %bdocker compose down%b                               # stop services\n" "${DIM}" "${RESET}"
  printf "    %bdocker compose pull && docker compose up -d%b       # update\n" "${DIM}" "${RESET}"
  echo
}
...
main() {
  pulse_logo
  printf "\n"
  printf "  %bInteractive installer for self-hosted Moneat%b\n" "${DIM}" "${RESET}"
  printf "  %bhttps://moneat.io/docs/self-hosting%b\n\n" "${DIM}" "${RESET}"
