...
-- * This file therefore treats current, recovery, and old staging tables as possible sources, keeps the
--   v46 recovery table, and uses a fresh rebuild table for each attempt.

CREATE TABLE IF NOT EXISTS security_package_inventory_v46_old (
...
INSERT INTO security_package_inventory_v46_old (
...
SELECT
    ...
    host, container_id, image_name, package_name, package_version, package_type, ecosystem,
    purl, licenses, supplier, bom_ref, tags, collected_at, ingested_at
FROM merge(currentDatabase(), '^security_package_inventory(_rekeyed)?$')
