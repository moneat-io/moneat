...
CREATE TABLE IF NOT EXISTS apm_spans (
    ...
    INDEX idx_apm_spans_service service TYPE bloom_filter GRANULARITY 1,
    INDEX idx_apm_spans_resource resource TYPE bloom_filter GRANULARITY 1,
    INDEX idx_apm_spans_name name TYPE bloom_filter GRANULARITY 1,
    INDEX idx_apm_spans_host host TYPE bloom_filter GRANULARITY 1
) ENGINE = MergeTree()
...
-- Trace stats table (pre-aggregated)
CREATE TABLE IF NOT EXISTS trace_stats (
    ...
    INDEX idx_trace_stats_service service TYPE bloom_filter GRANULARITY 1,
    INDEX idx_trace_stats_resource resource TYPE bloom_filter GRANULARITY 1
