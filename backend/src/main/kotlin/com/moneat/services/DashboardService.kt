...
class DashboardService {
    ...
    private suspend fun getReplayWindowErrorIds(
        ...
        return try {
            ...
            response.bodyAsText()
                ...
                .filter { it.isNotBlank() }
                .mapNotNull { line ->
                    runCatching { json.parseToJsonElement(line).jsonObject["event_id"]?.jsonPrimitive?.contentOrNull }.getOrNull()
                }
        } catch (e: Exception) {
    ...
    // ── Org-level aggregate helpers ────────────────────────────────────

    fun getOrgProjectIds(userId: Int): List<Long> {
        return transaction {
            val orgIds = Memberships.selectAll().where { Memberships.user_id eq userId }
                .map { it[Memberships.organization_id] }
            Projects.selectAll().where { Projects.organization_id inList orgIds }
    ...
    fun getOrgProjectNames(userId: Int): Map<Long, String> {
        return transaction {
            val orgIds = Memberships.selectAll().where { Memberships.user_id eq userId }
                .map { it[Memberships.organization_id] }
            Projects.selectAll().where { Projects.organization_id inList orgIds }
    ...
    private fun getOrgRetentionDays(userId: Int): Int {
        val orgId = transaction {
            Memberships.selectAll().where { Memberships.user_id eq userId }
                .firstOrNull()?.get(Memberships.organization_id)
        } ?: return PricingTier.FREE.retentionDays
        return kotlinx.coroutines.runBlocking { retentionPolicyService.getRetentionDaysForOrganization(orgId) }
    }

    private fun projectIdsInClause(projectIds: List<Long>): String {
        return "project_id IN (${projectIds.joinToString(",")})"
    }

    // ── Org-level aggregate endpoints ─────────────────────────────────
    ...
    suspend fun getOrgIssues(
        ...
        val projectIds = getOrgProjectIds(userId)
        if (projectIds.isEmpty()) return emptyList()
        val retentionDays = getOrgRetentionDays(userId)
        val offset = (page - 1) * limit
        val validStatuses = setOf("unresolved", "resolved", "ignored")
        val statusFilter = if (status != null && status in validStatuses) {
            "AND status = '${status.replace("'", "''")}'"
        } else ""
        val projectFilter = projectIdsInClause(projectIds)
        ...
        return try {
            ...
            val body = response.bodyAsText()
            if (!response.status.isSuccess() || body.trimStart().startsWith("Code:")) {
                logger.error { "Failed to fetch org issues: ${response.status} ${body.take(400)}" }
                return emptyList()
            }
            body.lines()
                .filter { it.isNotBlank() }
                .map { line ->
                    ...
                    IssueResponse(
                        id = obj["issue_id"]?.jsonPrimitive?.content ?: "",
                        projectId = obj["project_id"]?.jsonPrimitive?.long ?: 0,
                        title = obj["title"]?.jsonPrimitive?.content ?: "",
                        culprit = obj["culprit"]?.jsonPrimitive?.content ?: "",
                        level = obj["level"]?.jsonPrimitive?.content ?: "error",
                        platform = obj["platform"]?.jsonPrimitive?.content ?: "",
            ...
            logger.error(e) { "Failed to fetch org issues" }
            emptyList()
        }
    }

    suspend fun getOrgStats(userId: Int, period: String = "24h"): ProjectStatsResponse {
        val projectIds = getOrgProjectIds(userId)
        if (projectIds.isEmpty()) return ProjectStatsResponse(0, 0, 0, 0, emptyList(), emptyMap(), emptyMap(), emptyMap(), emptyMap(), emptyMap(), emptyList(), emptyList())
        val retentionDays = getOrgRetentionDays(userId)
        val hoursBack = when (period) {
            "24h" -> 24; "7d" -> 168; "30d" -> 720; "90d" -> 2160; else -> 168
        }
        val intervalMinutes = when (period) {
            "24h" -> 60; "7d" -> 360; "30d" -> 1440; "90d" -> 4320; else -> 360
        }
        val pf = projectIdsInClause(projectIds)

        return coroutineScope {
            val totalEvents = async { executeScalarQuery("""
                SELECT count() as total FROM $clickhouseDb.events
                WHERE $pf AND timestamp >= now() - INTERVAL $hoursBack HOUR
                AND ${timestampRetentionClause("timestamp", retentionDays)} FORMAT JSONEachRow
            """.trimIndent()) }
            val totalIssues = async { executeScalarQuery("""
                SELECT count(DISTINCT issue_id) as total FROM $clickhouseDb.events
                WHERE $pf AND event_type = 'error'
                AND ${timestampRetentionClause("timestamp", retentionDays)} FORMAT JSONEachRow
            """.trimIndent()) }
            val unresolvedIssues = async { executeScalarQuery("""
                SELECT count() as total FROM (
                    SELECT issue_id FROM $clickhouseDb.issues FINAL
                    WHERE $pf AND status = 'unresolved'
                    AND ${timestampRetentionClause("last_seen", retentionDays)}
                ) FORMAT JSONEachRow
            """.trimIndent()) }
            val affectedUsers = async { executeScalarQuery("""
                SELECT uniq(user_id) as total FROM $clickhouseDb.events
                WHERE $pf AND timestamp >= now() - INTERVAL $hoursBack HOUR
                AND user_id != '' AND ${timestampRetentionClause("timestamp", retentionDays)} FORMAT JSONEachRow
            """.trimIndent()) }
            val eventsTimeline = async { executeTimelineQuery("""
                SELECT toStartOfInterval(timestamp, INTERVAL $intervalMinutes MINUTE) as time, count() as count
                FROM $clickhouseDb.events WHERE $pf
                AND timestamp >= now() - INTERVAL $hoursBack HOUR
                AND ${timestampRetentionClause("timestamp", retentionDays)}
                GROUP BY time ORDER BY time FORMAT JSONEachRow
            """.trimIndent()) }

            ProjectStatsResponse(
                totalEvents = totalEvents.await(),
                totalIssues = totalIssues.await(),
                unresolvedIssues = unresolvedIssues.await(),
                affectedUsers = affectedUsers.await(),
                eventsTimeline = eventsTimeline.await(),
                eventsByLevel = emptyMap(),
                eventsByPlatform = emptyMap(),
                eventsByBrowser = emptyMap(),
                eventsByEnvironment = emptyMap(),
    ...
    suspend fun getOrgReleases(userId: Int): List<ReleaseListResponse> {
        val projectIds = getOrgProjectIds(userId)
        if (projectIds.isEmpty()) return emptyList()
        val retentionDays = getOrgRetentionDays(userId)
        val pf = projectIdsInClause(projectIds)
        ...
        return try {
            val releases = executeReleasesListQuery(query)
            releases.map { r ->
                ReleaseListResponse(
                    version = r.version,
                    firstSeen = r.firstSeen,
            ...
            logger.error(e) { "Failed to fetch org releases" }
            emptyList()
        }
    }

    suspend fun getOrgReplays(
        userId: Int,
        page: Int = 1,
        limit: Int = 25,
        period: String = "7d"
        ...
        val periodMs = when (period) {
            "24h" -> 24 * 60 * 60 * 1000L; "30d" -> 30 * 24 * 60 * 60 * 1000L
            "90d" -> 90 * 24 * 60 * 60 * 1000L; else -> 7 * 24 * 60 * 60 * 1000L
        }
        val periodStartMs = nowMs - periodMs
        val retentionStartMs = nowMs - (retentionDays * 24 * 60 * 60 * 1000L)
        val pf = projectIdsInClause(projectIds)
        ...
        return try {
            ...
            body.lines()
                ...
                .mapNotNull { line ->
                    try {
                        val obj = json.parseToJsonElement(line).jsonObject
                        val userId2 = obj["user_id"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                        val userEmail = obj["user_email"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                        val userUsername = obj["user_username"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                        ReplayListItem(
                            replayId = obj["replay_id"]?.jsonPrimitive?.content ?: "",
                            projectId = obj["project_id"]?.jsonPrimitive?.long ?: 0,
                            startedAt = obj["started_at"]?.jsonPrimitive?.content ?: "",
                            finishedAt = obj["finished_at"]?.jsonPrimitive?.content ?: "",
                            durationMs = obj["duration_ms"]?.jsonPrimitive?.contentOrNull?.toDoubleOrNull() ?: 0.0,
                            urls = (obj["urls"] as? kotlinx.serialization.json.JsonArray)?.mapNotNull { it.jsonPrimitive.contentOrNull } ?: emptyList(),
                            errorCount = obj["error_count"]?.jsonPrimitive?.contentOrNull?.toIntOrNull() ?: 0,
                            user = if (userId2 != null || userEmail != null || userUsername != null) UserInfo(id = userId2, email = userEmail, username = userUsername, ip_address = null) else null,
                            browserName = obj["browser_name"]?.jsonPrimitive?.contentOrNull,
                            browserVersion = obj["browser_version"]?.jsonPrimitive?.contentOrNull,
                            osName = obj["os_name"]?.jsonPrimitive?.contentOrNull,
                            osVersion = obj["os_version"]?.jsonPrimitive?.contentOrNull,
                            activity = obj["activity"]?.jsonPrimitive?.contentOrNull?.toIntOrNull() ?: 0
                        )
                    } catch (e: Exception) {
                        logger.error(e) { "Failed to parse replay: $line" }
                        null
                    }
                }
        } catch (e: Exception) {
            logger.error(e) { "Failed to fetch org replays" }
            emptyList()
        }
    }

    suspend fun getOrgFeedback(
        userId: Int,
        page: Int = 1,
        limit: Int = 25,
        status: String? = null
        ...
        val query = """
            SELECT
                toString(feedback_id) as feedback_id,
                message, contact_email, name, url, status,
                formatDateTime(timestamp, '%Y-%m-%dT%H:%i:%S.000Z') as created_at,
                environment, release, platform,
        ...
        return try {
            ...
            body.lines()
                ...
                .map { line ->
                    ...
                    val uemail = obj["user_email"]?.jsonPrimitive?.contentOrNull
                    val uname = obj["user_username"]?.jsonPrimitive?.contentOrNull
                    FeedbackListItem(
                        feedbackId = obj["feedback_id"]?.jsonPrimitive?.content ?: "",
                        message = obj["message"]?.jsonPrimitive?.content ?: "",
                        contactEmail = obj["contact_email"]?.jsonPrimitive?.content ?: "",
                        ...
                        release = obj["release"]?.jsonPrimitive?.content ?: "",
                        platform = obj["platform"]?.jsonPrimitive?.content ?: "",
                        user = if (uid != null || uemail != null || uname != null) UserInfo(id = uid, email = uemail, username = uname, ip_address = null) else null,
                        associatedEventId = obj["associated_event_id"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() },
                        replayId = obj["replay_id"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    )
                }
        } catch (e: Exception) {
            logger.error(e) { "Failed to fetch org feedback" }
            emptyList()
        }
    }

    suspend fun getOrgTransactions(
        userId: Int,
        period: String = "7d",
        environment: String? = null,
        operation: String? = null
        ...
        return try {
            ...
            val body = response.bodyAsText()
            if (!response.status.isSuccess() || body.trimStart().startsWith("Code:")) {
                logger.error { "Failed to fetch org transactions: ${response.status} ${body.take(400)}" }
                return emptyList()
            }
            body.lines()
                .filter { it.isNotBlank() }
            ...
            logger.error(e) { "Failed to fetch org transactions" }
            emptyList()
        }
    }

    suspend fun getOrgPerformanceStats(
        userId: Int,
        period: String = "7d",
        environment: String? = null,
        operation: String? = null
        ...
        val projectIds = getOrgProjectIds(userId)
        if (projectIds.isEmpty()) return PerformanceStatsResponse(0.0, emptyList(), emptyList(), 0, 0.0)
        val retentionDays = getOrgRetentionDays(userId)
        val periodConfig = getPeriodConfig(period)
        val filters = buildTransactionFilterClause(environment, operation)
        val pf = projectIdsInClause(projectIds)
        ...
        val throughputQuery = """
            SELECT
                formatDateTime(toStartOfInterval(e.timestamp, INTERVAL ${periodConfig.intervalMinutes} MINUTE), '%Y-%c-%dT%H:%i:%S.000Z') as time,
                count() as count
            FROM $clickhouseDb.events e
            WHERE $pf
                AND e.event_type = 'transaction'
        ...
        val slowestQuery = """
            ...
            WHERE $pf
                ...
                AND ${timestampRetentionClause("e.timestamp", retentionDays)}
                $filters
            ORDER BY e.duration_ms DESC LIMIT 10 FORMAT JSONEachRow
        """.trimIndent()
        ...
        return try {
            ...
            val aggregateBody = aggregateResponse.bodyAsText()
            if (!aggregateResponse.status.isSuccess() || aggregateBody.trimStart().startsWith("Code:")) {
                logger.error { "Failed to fetch org perf stats: ${aggregateResponse.status} ${aggregateBody.take(400)}" }
                return PerformanceStatsResponse(0.0, emptyList(), emptyList(), 0, 0.0)
            }
            val aggregateObj = aggregateBody.lines().firstOrNull { it.isNotBlank() }?.let {
                json.parseToJsonElement(it).jsonObject
            }
            ...
            logger.error(e) { "Failed to fetch org performance stats" }
            PerformanceStatsResponse(0.0, emptyList(), emptyList(), 0, 0.0)
        }
    }
}
