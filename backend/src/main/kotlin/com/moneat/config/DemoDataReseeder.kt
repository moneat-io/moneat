 ...
 * does not silently delete the one-time seed migrations (V6/V7/V8).
 *
...
object DemoDataReseeder {
    ...
    suspend fun reseedIfNeeded() {
        ...
        try {
            val freshCount = checkFreshDataCount()
            if (freshCount > 0) {
                logger.info { "Demo data looks fresh ($freshCount recent events), skipping reseed" }
                return
            }

            logger.info { "Demo data is stale or missing, reseeding..." }
            purgeOldDemoData()
            reseedEvents()
            reseedSessions()
            reseedReplays()
