...
fun Route.apiRoutes() {
    ...
    authenticate("auth-jwt") {
        ...
        route("/v1") {
            ...
            get("/organizations/issues") {
                val principal = call.principal<JWTPrincipal>()
                val userId = principal!!.payload.getClaim("userId").asInt()
                val page = call.request.queryParameters["page"]?.toIntOrNull() ?: 1
                val limit = call.request.queryParameters["limit"]?.toIntOrNull() ?: 25
                val status = call.request.queryParameters["status"]
                val issues = dashboardService.getOrgIssues(userId, page, limit, status)
                val projectNames = dashboardService.getOrgProjectNames(userId)
                call.respond(issues.map { issue ->
                    mapOf(
                        "id" to issue.id,
                        "projectId" to issue.projectId,
                        "projectName" to (projectNames[issue.projectId] ?: ""),
                        "title" to issue.title,
                        "culprit" to issue.culprit,
                        "level" to issue.level,
                        "platform" to issue.platform,
                        "firstSeen" to issue.firstSeen,
                        "lastSeen" to issue.lastSeen,
                        "eventCount" to issue.eventCount,
                        "userCount" to issue.userCount,
            ...
            get("/organizations/stats") {
                ...
                val stats = dashboardService.getOrgStats(userId, period)
                call.respond(stats)
            }

            get("/organizations/releases") {
                val principal = call.principal<JWTPrincipal>()
                val userId = principal!!.payload.getClaim("userId").asInt()
                val releases = dashboardService.getOrgReleases(userId)
                call.respond(releases)
            }

            get("/organizations/replays") {
                val principal = call.principal<JWTPrincipal>()
                val userId = principal!!.payload.getClaim("userId").asInt()
                val page = call.request.queryParameters["page"]?.toIntOrNull() ?: 1
                val limit = call.request.queryParameters["limit"]?.toIntOrNull() ?: 25
                val period = call.request.queryParameters["period"] ?: "7d"
                val replays = dashboardService.getOrgReplays(userId, page, limit, period)
                call.respond(replays)
            }

            get("/organizations/feedback") {
                val principal = call.principal<JWTPrincipal>()
                val userId = principal!!.payload.getClaim("userId").asInt()
                val page = call.request.queryParameters["page"]?.toIntOrNull() ?: 1
                val limit = call.request.queryParameters["limit"]?.toIntOrNull() ?: 25
                val status = call.request.queryParameters["status"]
                val feedback = dashboardService.getOrgFeedback(userId, page, limit, status)
                call.respond(feedback)
            }
            ...
            get("/organizations/performance") {
                ...
                val environment = call.request.queryParameters["environment"]
                val operation = call.request.queryParameters["operation"]
                val transactions = dashboardService.getOrgTransactions(userId, period, environment, operation)
                call.respond(transactions)
            }

            get("/organizations/performance/stats") {
                val principal = call.principal<JWTPrincipal>()
                val userId = principal!!.payload.getClaim("userId").asInt()
                val period = call.request.queryParameters["period"] ?: "7d"
                val environment = call.request.queryParameters["environment"]
                val operation = call.request.queryParameters["operation"]
                val stats = dashboardService.getOrgPerformanceStats(userId, period, environment, operation)
                call.respond(stats)
            }
        }
        }
