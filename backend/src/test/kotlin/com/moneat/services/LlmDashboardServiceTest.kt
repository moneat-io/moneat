...
class LlmDashboardServiceTest {
    ...
    @Test
    fun `getOverview parses stats timeline and top models`() =
        runBlocking {
    ...
    @Test
    fun `getGenerationDetail returns null when clickhouse returns exception body`() =
        runBlocking {
            MockHttpServer { exchange ->
    ...
    @Test
    fun `getTrace parses generations and computes totals`() =
        runBlocking {
            MockHttpServer { exchange ->
                val query = exchange.requestBodyText()
