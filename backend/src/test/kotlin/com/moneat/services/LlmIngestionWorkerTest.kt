...
class LlmIngestionWorkerTest {
    ...
    @Test
    fun `processMessageForTest sends bad payload to DLQ`() = runBlocking {
    ...
    @Test
    fun `insertGenerations builds clickhouse rows and maps unknown type to chat`() = runBlocking {
        ...
        MockHttpServer { exchange ->
            ...
            val generation = LlmGenerationIngest(
                traceId = "trace-1",
                spanId = "span-1",
                name = "gen",
                model = "gpt-4o-mini",
    ...
    @Test
    fun `insertGenerations throws when clickhouse insert fails`() = runBlocking {
        MockHttpServer { exchange ->
    ...
    @Test
    fun `processMessageForTest routes invalid json payload to DLQ`() = runBlocking {
        val worker = LlmIngestionWorker("llm:q", "llm:dlq", 1)
        val dlq = mutableListOf<String>()
