...
class DashboardServiceTest {
    ...
    @Test
    fun `getIssues applies pagination and status filter and parses rows`() = runBlocking {
    ...
    @Test
    fun `getIssue returns detail with latest event and demo project name`() = runBlocking {
        MockHttpServer { exchange ->
            val query = exchange.requestBodyText()
            when {
    ...
    @Test
    fun `getTraceDetails assembles span waterfall timing`() = runBlocking {
        MockHttpServer { exchange ->
            val query = exchange.requestBodyText()
    ...
    @Test
    fun `getProjectIdForEvent normalizes compact uuid`() = runBlocking {
        val queries = Collections.synchronizedList(mutableListOf<String>())
