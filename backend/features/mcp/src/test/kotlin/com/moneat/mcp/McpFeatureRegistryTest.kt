...
class McpFeatureRegistryTest {
    ...
    @Test
    fun `ServiceLoader discovers MCP module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("MCP" in moduleNames)
    }

    @Test
    fun `features response includes MCP when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("MCP" in response.modules)
    }
