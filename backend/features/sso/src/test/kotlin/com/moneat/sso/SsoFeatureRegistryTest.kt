...
class SsoFeatureRegistryTest {
    ...
    @Test
    fun `ServiceLoader discovers SSO module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("SSO" in moduleNames)
    }

    @Test
    fun `features response includes SSO when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("SSO" in response.modules)
    }
