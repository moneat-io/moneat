...
class SecurityServiceTest {
    ...
    private fun capturedQueuePayload(): String {
    ...
    private fun verifyNoQueueAdmission() {
        verify(exactly = 0) {
            mockRedisCommands.eval<String>(
                any<String>(),
                ScriptOutputType.VALUE,
    ...
    fun `enqueueSecurityEvents pushes to Redis and returns count`() {
        ...
        val decoded = SecurityIngestionService.decodeBatch(capturedQueuePayload())
    ...
    fun `insertBatch dispatches security events to ClickHouse`() = runBlocking {
        ...
        MockHttpServer { exchange ->
            ...
            val batch = QueuedSecurityBatch(
                ...
                batchType = "events",
                events = listOf(
                    com.moneat.security.ingestion.QueuedSecurityEventEntry(
                        ruleId = "r1",
                        ruleName = "Rule One",
                        ruleCategory = "threat",
                        severity = "high",
    ...
    fun `insertBatch dispatches activity dumps to ClickHouse`() = runBlocking {
        ...
        MockHttpServer { exchange ->
            ...
            val batch = QueuedSecurityBatch(
                ...
                batchType = "dumps",
                dumps = listOf(
                    com.moneat.security.ingestion.QueuedActivityDumpEntry(
                        activityType = "exec",
                        processName = "bash",
                        host = "srv-01",
                        durationNs = 500000,
    ...
    fun `insertBatch dispatches compliance findings to ClickHouse`() = runBlocking {
        ...
        MockHttpServer { exchange ->
            ...
            val batch = QueuedSecurityBatch(
                ...
                batchType = "findings",
                findings = listOf(
                    com.moneat.security.ingestion.QueuedComplianceEntry(
                        framework = "cis",
                        ruleId = "cis-1.1",
                        ruleName = "Root Check",
                        status = "failed",
