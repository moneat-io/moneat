...
class LlmFeatureRegistryTest {
    ...
    @Test
    fun `ServiceLoader discovers LLM module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("LLM" in moduleNames)
    }

    @Test
    fun `features response includes LLM when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("LLM" in response.modules)
    }
