...
class OverviewFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Overview module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Overview" in moduleNames)
    }

    @Test
    fun `features response includes Overview when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Overview" in response.modules)
    }
