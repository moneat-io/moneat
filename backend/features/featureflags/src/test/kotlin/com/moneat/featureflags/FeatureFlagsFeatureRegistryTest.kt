...
class FeatureFlagsFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Feature Flags module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Feature Flags" in moduleNames)
    }

    @Test
    fun `features response includes Feature Flags when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Feature Flags" in response.modules)
    }
