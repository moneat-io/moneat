...
class ContactFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Contact module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Contact" in moduleNames)
    }

    @Test
    fun `features response includes Contact when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Contact" in response.modules)
    }
