...
class AccountDeletionFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Account Deletion module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Account Deletion" in moduleNames)
    }

    @Test
    fun `features response includes Account Deletion when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Account Deletion" in response.modules)
    }
