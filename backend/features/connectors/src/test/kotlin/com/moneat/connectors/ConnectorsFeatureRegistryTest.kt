...
class ConnectorsFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Connectors module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Connectors" in moduleNames)
    }

    @Test
    fun `features response includes Connectors when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Connectors" in response.modules)
    }
