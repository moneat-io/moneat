...
class AnalyticsFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Analytics module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Analytics" in moduleNames)
    }

    @Test
    fun `features response includes Analytics when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Analytics" in response.modules)
    }
