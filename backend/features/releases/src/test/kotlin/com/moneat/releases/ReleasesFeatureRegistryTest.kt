...
class ReleasesFeatureRegistryTest {
    @BeforeTest
    fun resetBefore() {
    ...
    @Test
    fun `ServiceLoader discovers Releases module from feature resources`() {
        val moduleNames = ServiceLoader.load(EnterpriseModule::class.java)
            .map { module -> module.name }

        assertTrue("Releases" in moduleNames)
    }

    @Test
    fun `features response includes Releases when runtime module is present`() = testApplication {
        application {
        ...
        assertTrue("Releases" in response.modules)
    }
